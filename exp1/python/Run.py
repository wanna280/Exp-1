...
dp.confusion_matrix(Corr_Matrix)  #直接调用方法绘制混淆矩阵

# 5.根据相关矩阵，找到距离每个样本最近的三个样本，得到100x3的矩阵
data=np.array(_data) #
f=open('data_id.txt','w')  #创建文件data_id.txt的文件指针
for row in corr_Mat:   #遍历corr_Mat相关矩阵的每一行
    #heapq.nlargest(4,row)
    max_list=map(row.index,heapq.nlargest(4,row))
    max_list=list(max_list)
    res = []  #存放需要写入文件的内容
    for i in max_list[1:]: #遍历最大数的列表（排除掉它本身）
        res.append(data[int(i)][0])  #向res中附加data_rowArray[i][0]的内容
